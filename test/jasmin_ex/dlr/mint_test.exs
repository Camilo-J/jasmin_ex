...
defmodule JasminEx.Dlr.MintBrokerIntegrationTest do
  use ExUnit.Case, async: false
  ...
  test "broker retries one failed Mint callback and ACKs the successful redelivery" do
    ...
    processor = fn payload, meta ->
      outcome =
        HttpThrower.process(payload, meta,
          client: {MintClient, client_context},
          clock: {Clock, nil}
        )
